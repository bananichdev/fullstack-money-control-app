...
password_context = CryptContext(
    schemes=["bcrypt"], deprecated="auto", bcrypt__rounds=HASH_ROUNDS
)
...
async def check_token(access_token: Annotated[str, Depends(cookie_apikey)]) -> int:
    ...
    try:
        return int(jwt.decode(access_token, SECRET_KEY, ALGORITHM).get("id"))
    except JWTError as e:
        raise TokenIncorrect() from e


async def check_token_header(
    access_token: Annotated[str, Depends(header_apikey)]
) -> int:
    if access_token is None:
        raise TokenNotFound()
